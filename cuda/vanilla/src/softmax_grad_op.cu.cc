...
struct SoftmaxGradOpFunctor<GPUDevice, T, DType> {
  static void  Compute(OpKernelContext* context,
                       softmaxbackprop::SoftmaxGradParam<DType>& params,
                       int batch_size_,
                       int from_seq_len_,
                       int to_seq_len_,
                       int head_num_,
      ...
      softmaxbackprop::SoftmaxGrad<TFTraits<T>::OpType> *attention = nullptr;

      attention = new softmaxbackprop::SoftmaxGrad<TFTraits<T>::OpType>(allocator_, batch_size_,
...
#define REGISTER_KERNELS(T) \
    template struct functor::SoftmaxGradOpFunctor<GPUDevice, T, typename TFTraits<T>::DataType>;
