...
struct MultiHeadAttentionOpFunctor<GPUDevice, T, DType> {
  static void  Compute(OpKernelContext* context,
                        cuda::MultiHeadInitParam<DType>& params,
                        int batch_size_,
                        int from_seq_len_,
                        int to_seq_len_,
                        int head_num_,
      ...
      cuda::OpenMultiHeadAttention<TFTraits<T>::OpType> *attention = nullptr;

      attention = new cuda::OpenMultiHeadAttention<TFTraits<T>::OpType>(allocator_, batch_size_,
              from_seq_len_, to_seq_len_, head_num_, size_per_head_);
...
#define REGISTER_KERNELS(T) \
    template struct functor::MultiHeadAttentionOpFunctor<GPUDevice, T, typename TFTraits<T>::DataType>;
