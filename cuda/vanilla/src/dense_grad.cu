      ...
      )
{
    ...
    // TODO: Add leaky relu backprop
    if (OpType_ == OperationType::FP32) {
        launch_transpose1203_lreluback<float>((const float*)q_grads, (float*)grads_inter, (const float*)query_layer, param_.alpha,
                batch, from_seq_len, head_num, size_per_head, stream);

    } else {
        launch_transpose1203_lreluback<half>((const half*)q_grads, (half*)grads_inter, (const half*)query_layer, param_.alpha,
                batch, from_seq_len, head_num, size_per_head, stream);
    }

    // Computation of Query
    // [N, T_q, C]
    auto b_ptr = ToDeviceMemory(grads_inter);
    // [C_q, C]
    ...
    // [N, T_q, C]
    a_ptr = ToDeviceMemory(grads_inter);
    // [N, C_q, C]
    c_ptr = ToDeviceMemory(dw_inter);
    ...
    if (OpType_ == OperationType::FP32) {
        launch_sum_reduce_w<float>( (const float*)dw_inter,
                (float *)param_.dwq,
                batch,
                hidden_size,
                hs_q,
        ...
        launch_column_sum_reduce<float>(
                (const float *)grads_inter,
                (float *)param_.dbq,
                batch * from_seq_len,
                hidden_size,
                stream);
    } else {
        launch_sum_reduce_w<half>( (const half*)dw_inter,
                (half *)param_.dwq,
                batch,
                hidden_size,
                hs_q,
    ...
    // TODO: Add leaky relu backprop
    if (OpType_ == OperationType::FP32) {
        launch_transpose1203_lreluback<float>((const float*)k_grads, (float*)grads_inter, (const float*)key_layer, param_.alpha,
                batch, to_seq_len, head_num, size_per_head, stream);

    } else {
        launch_transpose1203_lreluback<half>((const half*)k_grads, (half*)grads_inter, (const half*)key_layer, param_.alpha,
                batch, to_seq_len, head_num, size_per_head, stream);
    }

    // Computation of Key
    // [N, T_k, C]
    b_ptr = ToDeviceMemory(grads_inter);
    // [C_k, C]
    ...
    if (OpType_ == OperationType::FP32) {
        ...
        launch_transpose1203_lreluback<float>((const float*)v_grads, (float*)grads_inter, (const float*)value_layer, param_.alpha,
                batch, to_seq_len, head_num, size_per_head_out, stream);

    } else {
        launch_transpose1203_lreluback<half>((const half*)v_grads, (half*)grads_inter, (const half*)value_layer, param_.alpha,
                batch, to_seq_len, head_num, size_per_head_out, stream);
    }
    ...
    // [N, C_v, O]
    //c_ptr = ToDeviceMemory(dw_inter);
    c_ptr = ToDeviceMemory(param_.dv_inter);
...
template void DenseGrad<OperationType::FP32>::denseBack_kernelLauncher(
        ...
        const float* query_layer, // input
        const float* key_layer, // input
...
template void DenseGrad<OperationType::FP16>::denseBack_kernelLauncher(
        ...
        const Eigen::half* query_layer, // input
        const Eigen::half* key_layer, // input
