                                   ...
                                   int width)
{
    ...
    for (int idy = blockIdx.y; idy < rows; idy += gridDim.y) {
        ...
        betta_tmp += (double)grad;
        gamma_tmp += (double)(val * grad);
    }

    if (idx < width) {
        int id = blockIdx.y * width + idx;
        betta_inter[id] = (T)betta_tmp;
        gamma_inter[id] = (T)gamma_tmp;
    }
}
...
__global__ void LayerNormBackward1_1(
    ...
    for (int r = threadIdx.y; r < rows; r += TILE_DIM) {
        ...
        betta_tmp += (double)betta;
        gamma_tmp += (double)gamma;

        offset += y_stride;
    }

    betta_buffer[threadIdx.x][threadIdx.y] = (float)betta_tmp;
    gamma_buffer[threadIdx.x][threadIdx.y] = (float)gamma_tmp;

    __syncthreads();

    // Sum the shared buffer.
    double s1 = (double)betta_buffer[threadIdx.y][threadIdx.x];
    double s2 = (double)gamma_buffer[threadIdx.y][threadIdx.x];
                                   ...
                                   float alpha)
{
    ...
    float vals_arr;
    float gamma_reg = gamma[laneId];
    vals_arr = out_grad[laneId];
    vals_arr *= gamma_reg;
    ...
    for (int i = 1; i < WARP_SIZE; i *= 2) { sum += g.shfl_down(sum, i); }
    sum = g.shfl(sum, 0);
    sum /= row_stride;  // dlmu/size_per_head ?

    int seq_id = row % seq_len;
    int batch_id = row / seq_len % batch;
    int head_id = row / (seq_len * batch);
    //inp_grad -= (row * row_stride);
