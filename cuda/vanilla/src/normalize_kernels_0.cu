                                   ...
                                   float alpha)
{
    ...
    float vals_arr[NORM_REG];
    int high_index = 1 * 32 + id;
#pragma unroll
    for (int i = 0; i < 1; i++) {
        float gamma_reg = gamma[i * 32 + id];
        vals_arr[i] = out_grad[i * 32 + id];
        vals_arr[i] *= gamma_reg;
    }
    ...
    sum = 0;
    for (int i = 0; i < 1; i++) { sum += vals_arr[i]; }
    ...
    for (int i = 0; i < 1; i++) {
        ...
        if (X_vals[i * 32 + id] < 0.0f)
            tmp = alpha * tmp;
        inp_grad[i * 32 + id] = tmp;
    }
}
