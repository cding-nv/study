...
struct DenseGradOpFunctor<GPUDevice, T, DType> {
  static void  Compute(OpKernelContext* context,
                       densebackprop::DenseGradParam<DType>& params,
                       int batch_size_,
                       int from_seq_len_,
                       int to_seq_len_,
                       int head_num_,
      ...
      densebackprop::DenseGrad<TFTraits<T>::OpType> *attention = nullptr;

      attention = new densebackprop::DenseGrad<TFTraits<T>::OpType>(allocator_, batch_size_,
              from_seq_len_, to_seq_len_, head_num_, hidden_units_, size_per_head_, size_per_head_out_,
              hs_q_, hs_k_, hs_v_);
...
#define REGISTER_KERNELS(T) \
    template struct functor::DenseGradOpFunctor<GPUDevice, T, typename TFTraits<T>::DataType>;
