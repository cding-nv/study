...
template <typename T>
__global__ void LayerNormBackward1_not32(const T* __restrict__ out_grad,
                                   const T* __restrict__ X_data,
                                   const T* __restrict__ vars,
                                   const T* __restrict__ means,
                                   T* __restrict__ gamma_grad,
                                   ...
                                   int row_stride)
{
    ...
    int id = threadIdx.x;
    int wid = id / WARP_SIZE;
    // TODO: row_stride should be divided by 32???
    // should THREADS will replaced by blockDim.x???
    int warp_num = (THREADS < row_stride ? THREADS : row_stride) / WARP_SIZE;
    __shared__ float partialSum[MAX_WARP_NUM];
...
#pragma unroll
    ...
    // TODO: to cope with the case when row_stride cannot be divided by iteration_stride
    if ((high_index) < row_stride) {
        float gamma_reg = gamma[high_index];
        vals_arr[iterations] = out_grad[high_index];
        vals_arr[iterations] *= gamma_reg;
...
// dishengbin
// This implentation requires: hidden_dim % 32 == 0
template <>
void launch_layerNorm_backward<float>(const float* out_grad,
                                      const float* X_data,
                                      const float* vars,
                                      ...
                                      cudaStream_t stream)
{
    ...
    if (hidden_dim % 32 == 0) {
        ...
        LayerNormBackward1_not32<float><<<grid_dim, block_dim, 0, stream>>>(
                out_grad, X_data, vars, means, gamma_grad, betta_grad, batch, hidden_dim);
