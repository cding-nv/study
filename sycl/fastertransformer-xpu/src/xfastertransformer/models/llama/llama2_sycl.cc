...
static void malloc_weights(TransformerWeights *w, Config *p, int shared_weights) {
    sycl::queue& q_ct1 = get_default_queue();
...
static void free_weights(TransformerWeights *w, int shared_weights) {
    sycl::queue& q_ct1 = get_default_queue();
...
static int load_weight(void* w, int elements, FILE* f, void* scratchCPU) {
...
static int load_checkpoint_weights(TransformerWeights *w, Config *p, FILE *f, int shared_weights) {
    sycl::queue& q_ct1 = get_default_queue();
...
static void build_transformer(Transformer* t, char* checkpoint_path) {
    ...
    t->config.vocab_size = std::abs(t->config.vocab_size);
...
static void forward(Transformer* transformer, int token, int pos) {
...
static int sample(Sampler* sampler, RunState* state) {
...
static void generate(Transformer *transformer, Sampler *sampler, int steps, int *prompt_tokens, int input_len, int *output_tokens) {
    //char *empty_prompt = "";
    if (prompt_tokens == NULL) { printf("Error: prompt_tokens is NULL\n"); exit(EXIT_FAILURE);}

    // encode the (string) prompt into tokens sequence
    int num_prompt_tokens = input_len;  // TODO ??
...
void llama_sycl_init(Transformer* transformer, Sampler* sampler, char* model_path){
    ...
    printf("#### llama_sycl_init");
    // default parameters
    //char *checkpoint_path = (char*)model_dir.c_str();
    //printf("#### model path: %s\n", checkpoint_path);
    //char *tokenizer_path = "/home/fengding/github/llama2.c/tokenizer.bin";
...
void llama_sycl_forward(Transformer* transformer, Sampler* sampler, int* output_tokens, int output_len, int* prompt_tokens, int input_len) {
    int steps = output_len ;//- input_len;            // number of steps to run for
    if (steps < 0) steps = 0;
    if (steps == 0 || steps > transformer->config.seq_len) steps = transformer->config.seq_len; // ovrerride to ~max length
    printf("#### steps = %d\n", steps);
    generate(transformer, sampler, steps, prompt_tokens, input_len, output_tokens);
}
