...
#endif
const __SYCL_CONSTANT_AS char format[] = "%.3f ";
const __SYCL_CONSTANT_AS char format_two[] = "%.3f %.3f";
const __SYCL_CONSTANT_AS char format_n[] = "\n";

#include "ifmha_policy_v4.h"
#include "ifmha_utils_v3.h"
#include "xetla.hpp"
...
template <typename ifmha_policy, typename scalar_t, bool kUseAlibi,
          bool kUseBias, bool kIsTraining>
...
public:
  ...
  struct arguments_t {
    ...
    inline arguments_t(scalar_t *query, scalar_t *key0, scalar_t *key1,
                       scalar_t *value0, scalar_t *value1, index_t *index,
                       scalar_t *alibi, scalar_t *bias, uint8_t *dropout,
                       accum_t dropout_prob, accum_t sm_scale, scalar_t *out,
                       uint32_t num_batches, uint32_t num_heads,
                       uint32_t head_size, uint32_t kv_len0, uint32_t kv_len1,
                       uint32_t padded_kvlen)
        : Q_ptr(query), K0_ptr(key0), K1_ptr(key1), V0_ptr(value0),
          V1_ptr(value1), I_ptr(index), A_ptr(alibi), B_ptr(bias),
          Dp_ptr(dropout), dp_prob(dropout_prob),
          dp_scale(1.f / (1.f - dropout_prob)), sm_scale(sm_scale), O_ptr(out),
          uB(num_batches), uN(num_heads), uH(head_size), uT0(kv_len0),
          uT1(kv_len1), uPT(padded_kvlen) {}
  };
...
private:
  ...
  static_assert(Beams * wg_size_x <= 32,
  ...
  static_assert(accum_step_1d % reduce_size == 0,
  ...
  struct context_t {
    ...
    inline void init_context(xetla_exec_item<2> &ei, arguments_t &args) {
      ...
      if constexpr (kUseBias){
        bias_base_offset = 
          batch_id * Beams * args.uPT + beam_id * args.uPT;
      };
      // (b, bm, N, 1, t)
      if constexpr (kUseAlibi){
        alibi_base_offset = batch_id * Beams * args.uN * (args.uT0 + args.uT1) +
          beam_id * args.uN * (args.uT0 + args.uT1) + head_id * (args.uT0 + args.uT1);
      }
    }
    // (b, bm, 1, 1, t)
    inline void set_bias_desc(arguments_t &args, int32_t start_T) {
      if constexpr (kUseBias) {
...
#if BTNH == 0
      int32_t start_x = batch_id * args.uN * args.uH + head_id * args.uH;
      int32_t start_y = start_T;
      uint32_t width = start_x + args.uH;
      uint32_t height = (start_y + kBc) < args.uT0 ? (start_y + kBc) : args.uT0;
      uint32_t pitch = args.uB * args.uN * args.uH;
    ...
    inline void set_context1(arguments_t &args, int32_t start_T) {
      ...
      using index_payload_t =
          subgroup::mem_payload_t<index_t, index_tile_desc_t,
                                  msg_type::block_2d, mem_desc_Ij_t::layout,
                                  mem_desc_Ij_t::space>;
  ...
  using brgemm_Sij_t = group::brgemm_t<compute_policy_bmbc, tile_shape_BmBc,
                                       mem_desc_QiL_t, mem_desc_Kj_t>;
  using matSij_t = typename brgemm_Sij_t::matAcc_t;

  template <typename matA_t, typename matB_t, typename matSrc_t,
            typename matDst_t>
...
#pragma unroll
        for (int mma_i = 0; mma_i < block_size_m / mma_m; mma_i++) {
          ...
          { // k=0
            ...
            dst_sub_blk =
                xetla_mma<gpu::xetla::detail::mma_argument_type<dtype_b>(),
...
#pragma unroll
  ...
  inline void gemm0_Sij(matQ_t &matQ, matSij_t &matSij, arguments_t &args) {
    ...
    const uint32_t loop_count =
        (args.uH + accum_step_bmbc - 1) / accum_step_bmbc;

    using matA_tile_desc_t =
        subgroup::tile_desc_t<tile_size_x_a, tile_size_y_a, block_size_x_a,
                              block_size_y_a, reg_layout::tiled>;
    using matA_t = subgroup::tile_t<scalar_t, matA_tile_desc_t>;
    using matB_tile_desc_t =
        subgroup::tile_desc_t<tile_size_x_b, tile_size_y_b, block_size_x_b,
                              block_size_y_b, reg_layout::vnni_tiled>;
    using matB_t = subgroup::tile_t<scalar_t, matB_tile_desc_t>;
    using matB_payload_t = subgroup::mem_payload_t<
        scalar_t, matB_tile_desc_t,
        subgroup::msg_type_v<matB_tile_desc_t, mem_space::global>, layout_b,
        mem_space::global, gpu_arch::Xe>;
    using matB_prefetch_payload_t = subgroup::prefetch_payload_t<
        scalar_t, subgroup::tile_desc_t<kSgBc, accum_step_bmbc, 1, 1>, layout_b,
        mem_space::global, tile_shape_BmBc::wg_size_y, gpu_arch::Xe>;
    ...
    xetla_nbarrier_t<tile_shape_BmBc::wg_size_y, tile_shape_BmBc::wg_size_y>
        nbarrier_b;
    nbarrier_b.init_nbarrier(sg_idx + nbarrier_cnt,
...
#pragma unroll
    ...
    for (int i = 0; i < loop_count; i++) {
      ...
      subgroup::tile_load<cache_hint::cached, cache_hint::cached>(matB,
                                                                  matB_payload);
  ...
  inline void gemm1_Sij(matQ_t &matQ, matSij_t &matSij, arguments_t &args) {
    ...
    using matQi_payload_t =
        subgroup::mem_payload_t<scalar_t, matQi_tile_desc_t, msg_type::block_1d,
                                mem_desc_QiL_t::layout, mem_desc_QiL_t::space>;
    ...
    using matQK_tile_desc_t = subgroup::tile_desc_t<reduce_size, block_size_x,
                                                    reduce_size, block_size_x>;
...
#pragma unroll
          ...
          src_sub += recur_col_reduce<reduce_op::sum, accum_t, reduce_size,
  ...
  using brgemm_Oi_t = group::brgemm_t<compute_policy_bmhm, tile_shape_BmHm,
                                      mem_desc_Pij_t, mem_desc_Vj_t>;
  ...
  inline void gemm0_Oi(matOi_t &matOi, arguments_t &args, int32_t start_T) {
    ...
    using matPi_load_t =
        subgroup::mem_payload_t<scalar_t, matPi_t, msg_type::block_1d,
                                mem_desc_Pij_t::layout, mem_desc_Pij_t::space>;
    ...
    using matB_tile_desc_t =
        subgroup::tile_desc_t<tile_size_x_b, tile_size_y_b, block_size_x_b,
                              block_size_y_b, reg_layout::vnni_tiled>;
    using matB_t = subgroup::tile_t<scalar_t, matB_tile_desc_t>;
    using matB_payload_t =
        subgroup::mem_payload_t<scalar_t, matB_tile_desc_t, msg_type::block_2d,
                                layout_b, mem_space::global, gpu_arch::Xe>;
    using matB_prefetch_payload_t = subgroup::prefetch_payload_t<
        scalar_t, subgroup::tile_desc_t<kSgBc, accum_step_bmhm, 1, 1>, layout_b,
        mem_space::global, tile_shape_BmHm::wg_size_y, gpu_arch::Xe>;
...
#pragma unroll
    for (int i = 0; i < loop_count; i++) {
      ...
      SW_BARRIER();
      if constexpr (sync_freq_bmhm > 0) {
        if constexpr ((i % sync_freq_bmhm) == 0) {
          if constexpr (tile_shape_BmHm::wg_size_y > 1) {
            nbarrier_b.wait();
          }
        }
...
#pragma unroll
          for (int k = 0; k < block_size_x; k++) {
            ...
            accum_t tmp = matPij.reg.xetla_select<1, 1>(j * block_elems +
                                                        i * block_size_x + k);
    ...
    if constexpr (wg_size_x > 1) {
      ...
      using matX_payload_t =
          subgroup::mem_payload_t<accum_t, matX_tile_desc_t, msg_type::block_1d,
      ...
      matX_payload_t matX_payload(PV_slm, kHm, ctx.beam_id + 1, kHm, offset,
                                  ctx.beam_id);
...
#pragma unroll
  ...
  inline void pre_softmax(matSij_t &matSij,
                          arguments_t &args, int32_t start_T, int32_t end_T) {
    // Multiply by softmax scaling factor
    matSij.reg *= args.sm_scale;

    if constexpr (kUseAlibi){
      matSij_t matAlibi(0);
      load_bias(args, args.A_ptr, ctx.alibi_base_offset, start_T, end_T, matAlibi);
      matSij.reg += matAlibi.reg;      
    }
    
    if constexpr (kUseBias) {
      matSij_t matBias(0);
      load_bias(args, args.B_ptr, ctx.bias_base_offset, start_T, end_T, matBias);            
  ...
  inline void softmax_fwd(matSij_t &matSij, matOi_t &matOi) {
    ...
    matSij.reg = xetla_exp<accum_t, kBm * kSgBc>(matSij.reg);
    if constexpr (wg_size_x > 1)
      ctx.nbarrier.wait();
  ...
  inline void store_Pij(matPij_t &matPij) {
    ...
    using epilogue_t = group::epilogue_t<
        ...
        tile_shape_BmBc, mem_desc_Pij_t>;
    epilogue_t epilogue;
    epilogue(ctx.g, matPij, ctx.desc_Pij);
#else    
    static constexpr uint32_t sg_tile_m = matPij_t::tile_size_y;
    static constexpr uint32_t sg_tile_n = matPij_t::tile_size_x;
    static constexpr uint32_t wg_size_x = tile_shape_BmBc::wg_size_x;
    static constexpr uint32_t wg_size_y = tile_shape_BmBc::wg_size_y;

    int32_t sg_idx = ctx.g.get_id() % wg_size_x;
    int32_t sg_idy = ctx.g.get_id() / wg_size_x;
    int32_t tile_offset_n = sg_idx * sg_tile_n;
    int32_t tile_offset_m = sg_idy * sg_tile_m;

    mem_desc_Pij_t mem_desc_c = ctx.desc_Pij;
    mem_desc_c.update_coord(tile_offset_n, tile_offset_m);

    using matC_tile_desc_t = subgroup::tile_desc_t<matPij_t::tile_size_x,
                matPij_t::tile_size_y, matPij_t::block_size_x,
                matPij_t::block_size_y, reg_layout::tiled>;
    using matC_t = subgroup::tile_t<typename mem_desc_Pij_t::dtype, matC_tile_desc_t>;
    using matC_payload_t = subgroup::mem_payload_t<typename mem_desc_Pij_t::dtype, matC_tile_desc_t, msg_type::block_1d,
                        mem_desc_Pij_t::layout, mem_desc_Pij_t::space, gpu_arch::Xe>;
    matC_payload_t matC_payload(mem_desc_c);
    matC_t matC;
    subgroup::elemwise_cvt(matC, matPij);
  ...
  inline void preload_Qi(matQ_t &matQ) {
    using matQi_load_t =
        subgroup::mem_payload_t<scalar_t, matQ_tile_desc_t, msg_type::block_1d,
                                mem_desc_Qi_t::layout, mem_desc_Qi_t::space>;
...
public:
  ...
  get_nd_range(uint32_t num_batches, uint32_t num_beams, uint32_t num_heads) {
  ...
  inline KERNEL_FUNC void print_matsij(xetla_vector<float, kSgBc> &mat) {
    if (ctx.sg_idx == 0) {
      for (int i = 0; i < kSgBc; ++i) {
        auto data_view = mat.xetla_select<1, 1>(i);
        accum_t data = data_view[0];
        sycl::ext::oneapi::experimental::printf(format, float(data));
      }
      sycl::ext::oneapi::experimental::printf(format_n);
    }
  }
  // bias:（b, beams, 1, 1, t)
  // alibi: (b, beams, N, 1, t)
  inline KERNEL_FUNC void load_bias(arguments_t &args, scalar_t* ptr, uint32_t base_offset, uint32_t startT,
                                    uint32_t endT, matSij_t &matAcc) {
...
#pragma unroll
    for (int i = 0; i < loops; ++i) {
      ...
      matAcc.reg.xetla_select<simd_lanes, 1>(i * simd_lanes) =
          xetla_load_global<scalar_t, 1, data_size::default_size,
                            cache_hint::cached, cache_hint::cached, simd_lanes>(
              ptr, offsets);
    }
  }

  inline KERNEL_FUNC void operator()(xetla_exec_item<2> &ei,
                                     arguments_t &args) {
    ...
    for (int32_t start_T = 0; start_T < args.uT0; start_T += kBc) {
      ...
      // softmax
      pre_softmax(matSij, args, start_T, args.uT0);
    ...
    for (int32_t start_T = 0; start_T < args.uT1; start_T += kBc) {
      ...
      // softmax
      pre_softmax(matSij, args, start_T + args.uT0, args.uT0 + args.uT1);
      softmax_fwd(matSij, matOi);
      // compute Oi
