...
struct imem_desc_t {
  ...
  template <typename tile_t> inline void iload_tile(tile_t &tile) {
    using dtype_acc = typename tile_t::dtype;
    using tile_desc = typename tile_t::tile_desc;
    static_assert(tile_desc::tile_size_x == LoadSize,
    ...
    using lane_payload_t =
        subgroup::mem_payload_t<dtype, lane_tile_desc_t, msg_type::block_1d,
                                layout, space>;
    ...
    if (offset_ < width_ && lane_ < total_) {
      ...
      lane_payload_t lane_payload((dtype*)addr_, width_, idx + 1, pitch_, offset_, idx);
  ...
  inline void iprefetch_tile() {
    using lane_tile_desc_t = subgroup::tile_desc_t<LoadSize, 1, 16, 1>;
    using lane_payload_t =
        subgroup::prefetch_payload_t<dtype, lane_tile_desc_t, layout, space, 1>;

    if (offset_pre_ < width_ && lane_pre_ < total_) {
      int32_t idx = index_[lane_pre_ * Beams + beam_pre_];
      lane_payload_t lane_payload((dtype*)addr_, width_, idx + 1, pitch_, offset_pre_,
                                  idx);
...
inline xetla_vector<T, reduce_size> partial_reduce(xetla_vector<T, size> src) {
  static_assert(size % reduce_size == 0,
                "size should be a multiple of reduce_size");
...
struct group_row_reduce_t {
  ...
  inline KERNEL_FUNC xetla_vector<T, kNum> operator()(mat_t &src) {
    xetla_vector<T, kNum> ret =
        subgroup::tile_reduce<reduce_kind, T, T, 1, mat_t>(src);
