...
#include "src/xfastertransformer/th_op/llama/LlamaOp.h"
                                           ...
                                           th::optional<int64_t>    return_cum_log_probs_opt)
{
    ...
    const int  total_request_output_len = output_len; //max_input_length + output_len;
    printf("#### output_ids shape, batch_siz=%d, beam_width=%d, total_request_output_len=%d (max_input_length=%d output_len=%ld)\n", batch_size, beam_width, total_request_output_len, max_input_length, output_len);
    ...
    th::Tensor sequence_lengths =
        torch::empty({batch_size, beam_width}, torch::dtype(torch::kInt32).requires_grad(false));
    //sequence_lengths = sequence_lengths.to("xpu");
    th::Tensor cum_log_probs =
        torch::empty({batch_size, beam_width}, torch::dtype(torch::kFloat32).requires_grad(false));
    cum_log_probs = cum_log_probs.to("xpu");
