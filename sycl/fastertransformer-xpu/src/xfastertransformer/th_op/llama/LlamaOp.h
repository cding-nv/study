...
#include "src/xfastertransformer/models/llama/llama2_sycl.h"
#include "src/xfastertransformer/models/llama/common/common.h"
#include "src/xfastertransformer/models/llama/common/sampler.h"
//#include "src/xfastertransformer/th_op/th_utils.h"
...
public:
    ...
    {
        printf("#### FTLlama create\n");
        INIReader reader = INIReader("./llama_config.ini");
        std::string model_dir = std::string(reader.Get("ft_instance_hyperparameter", "model_dir"));
        char *checkpoint_path = (char*)model_dir.c_str();
        printf("#### model path: %s\n", checkpoint_path);

        std::string weight_data_type = std::string(reader.Get("ft_instance_hyperparameter", "weight_data_type"));

        std::cout << "#### weights_type " << weight_data_type << std::endl;

        transformer = (Transformer*)malloc(sizeof(Transformer));
        sampler = (Sampler*)malloc(sizeof(Sampler));

        if (weight_data_type.compare("fp16") == 0) {
            weights_type = WeightsType::fp16;
            ft::llama_sycl_init(transformer, sampler, checkpoint_path);
        } else if (weight_data_type.compare("int4") == 0) {
            weights_type = WeightsType::int4;
            //ft::llama_sycl_q4_init(transformer, sampler, checkpoint_path);
        }
    ...
    {
        printf("#### FTLlama forward\n");
        int* input_len = get_ptr<int>(input_lengths);
        printf("    #### seq_len = %d, request_output_len=%d\n", input_len[0], request_output_len);

        int* prompt_tokens = get_ptr<int>(input_ids);
        int* output_tokens = get_ptr<int>(output_ids);
        if (weights_type == WeightsType::fp16) {
            ft::llama_sycl_forward(transformer, sampler, output_tokens, request_output_len, prompt_tokens, *input_len);
        } else if (weights_type == WeightsType::int4) {
            //ft::llama_sycl_q4_forward(transformer, sampler, output_tokens, request_output_len, prompt_tokens, *input_len);
        }
        // TODO:   sequence_lengths
