...
class LlamaWeights(object):
    def __init__(self, 
        ...
        self.w = []
        # Transformer blocks
        print("#### global_hidden_units ", global_hidden_units, " inference_data_type ", inference_data_type, " layer_num ", layer_num)
...
class Llama(nn.Module):
    def __init__(self,
        ...
        self.device = 0 #self.rank % self.device_count
        ...
        self.tensor_para_rank = 0 #self.rank % self.tensor_para_size
        self.pipeline_para_rank = 0 #self.rank // self.tensor_para_size
    ...
    def cuda(self):
        ...
        if self.build_model:
            del self.model
            self.build_model = False
        print("#### torch.classes.FasterTransformer.LlamaOp")
    ...
    def forward(self,
        ...
        print("#### llama.py forward")

        # Inputs to device
        #input_ids = start_ids.to("xpu")#(self.device)
        #input_lengths = start_lengths.to("xpu")#(self.device)
        # outputs: output_ids, output_lengths, output_cum_log_probs (optional)
        print("#### llama.py output_len ", output_len)
